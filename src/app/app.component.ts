...
export class AppComponent {
  ...
  insertBook(e:Book){
    const existingCartItemIndex = this.cartItems.findIndex(obj => obj.bookInfo.ISBN === e.ISBN);
    if (existingCartItemIndex !== -1) {
      this.priceIncrease(e.ISBN);
    }
    else{
      ...
      this.totalPrice=this.totalPrice + e.price.value;
    }
    //console.log(e);
  }
  priceIncrease(id:number){
    let itemIndex=this.cartItems.findIndex(obj=>obj.bookInfo.ISBN===id);
    this.cartItems[itemIndex].quantity++;
    this.totalPrice=this.totalPrice + this.cartItems[itemIndex].bookInfo.price.value;
  }
  priceDecrease(id:number){
    let itemIndex=this.cartItems.findIndex(obj=>obj.bookInfo.ISBN===id);
  ...
  removeItem(id:number){
    let itemIndex=this.cartItems.findIndex(obj=>obj.bookInfo.ISBN===id);
    this.totalPrice=this.totalPrice - (this.cartItems[itemIndex].quantity)*(this.cartItems[itemIndex].bookInfo.price.value);
